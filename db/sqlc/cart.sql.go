// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.20.0
// source: cart.sql

package db

import (
	"context"
)

const createCart = `-- name: CreateCart :one
INSERT INTO carts (
    username
) VALUES (
    $1
)
RETURNING id, username
`

func (q *Queries) CreateCart(ctx context.Context, username string) (Cart, error) {
	row := q.db.QueryRow(ctx, createCart, username)
	var i Cart
	err := row.Scan(&i.ID, &i.Username)
	return i, err
}

const deleteCart = `-- name: DeleteCart :exec
DELETE FROM carts
WHERE username = $1
`

func (q *Queries) DeleteCart(ctx context.Context, username string) error {
	_, err := q.db.Exec(ctx, deleteCart, username)
	return err
}

const getCartByID = `-- name: GetCartByID :one
SELECT id, username FROM carts
WHERE id = $1
`

func (q *Queries) GetCartByID(ctx context.Context, id int64) (Cart, error) {
	row := q.db.QueryRow(ctx, getCartByID, id)
	var i Cart
	err := row.Scan(&i.ID, &i.Username)
	return i, err
}

const getCartByUsername = `-- name: GetCartByUsername :one
SELECT id, username FROM carts
WHERE username = $1
`

func (q *Queries) GetCartByUsername(ctx context.Context, username string) (Cart, error) {
	row := q.db.QueryRow(ctx, getCartByUsername, username)
	var i Cart
	err := row.Scan(&i.ID, &i.Username)
	return i, err
}
